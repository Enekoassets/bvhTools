...
##### `removeLimb(bvhData: BVHData, limbName: str) -> BVHData`
With this method you can choose a bone (it can't be the root bone), and remove it and all it's children **both** from the **hierarchy and** from the **motion data** automatically. All the other motion data is going to be left untouched.
...
Then, for this example, the bvhDataNoLeg skeleton would contain 3 less bones after removing the **leftLeg** and the bvhDataNoLeg motion would contain 9 less rotation columns (3 rotations * 3 bones).
...
##### `scaleSkeleton(bvhData: BVHData, scaleFactor: float) -> BVHData`

With this function you can scale the entire skeleton by a factor (the factor has to be greater than 0). Internally, this method will scale the offset values of each bone, as well as the position coordinates of the root bone in all the motion frames.
